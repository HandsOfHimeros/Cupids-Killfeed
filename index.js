...
// Batch spawn multiple entries in a single FTP operation to avoid race conditions
async function addCupidSpawnEntriesBatch(spawnEntries, guildId) {
    if (!spawnEntries || spawnEntries.length === 0) {
        console.log('[SPAWN BATCH] No entries to spawn');
        return [];
    }

    console.log(`[SPAWN BATCH] Processing ${spawnEntries.length} spawn entries...`);
    const results = [];
    
    const guildConfig = await db.getGuildConfig(guildId);
    if (!guildConfig || !guildConfig.nitrado_token || !guildConfig.nitrado_service_id) {
        throw new Error('Guild not configured for Nitrado');
    }

    const FILE_PATH = `/dayzps_missions/dayzOffline.${guildConfig.selected_map}/custom/spawn.json`;
    const GAMEPLAY_FILE_PATH = `/dayzps_missions/dayzOffline.${guildConfig.selected_map}/cfggameplay.json`;
    const BASE_URL = 'https://api.nitrado.net/services';
    ...
    try {
        ...
        try {
            ...
            console.error('[SPAWN BATCH] Error loading spawn templates:', err.message);
        }
        ...
        try {
            ...
            console.log(`[SPAWN BATCH] Downloaded spawn.json with ${spawnJson.Objects.length} existing objects`);
        } catch (downloadErr) {
            console.log('[SPAWN BATCH] Could not download spawn.json, creating new');
        }

        // Get player location (use first entry's player as reference)
        const firstEntry = spawnEntries[0];
        let playerLocation = null;
        try {
            const loc = await db.getPlayerPosition(guildId, firstEntry.dayzPlayerName);
            if (loc) {
                playerLocation = { x: loc.x, y: loc.y, z: loc.z };
                console.log(`[SPAWN BATCH] Player location: [${loc.x}, ${loc.y}, ${loc.z}]`);
            }
        } catch (err) {
            console.log('[SPAWN BATCH] Could not get player location');
        }

        if (!playerLocation) {
            throw new Error('Player location not found - make sure you are in-game and /imhere has been used');
        }

        // Find or create nearby table
        let nearbyTable = spawnJson.Objects.find(obj => {
            if (obj.name !== 'Land_Table_small_1') return false;
            const dx = obj.pos[0] - playerLocation.x;
            const dz = obj.pos[2] - playerLocation.z;
        ...
        if (!nearbyTable) {
            tablePos = [playerLocation.x + 1, playerLocation.y - 0.9, playerLocation.z];
            const tableObject = {
                name: 'Land_Table_small_1',
                pos: tablePos,
                ypr: [0, 0, 0],
                scale: 1,
                enableCEPersistency: 0,
                customString: JSON.stringify({
                    owner: firstEntry.dayzPlayerName,
                    created: Date.now(),
                    restart_id: firstEntry.restart_id,
                    item_count: 0
                })
            };
            spawnJson.Objects.push(tableObject);
            nearbyTable = tableObject;
            console.log(`[SPAWN BATCH] Created new table at [${tablePos[0]}, ${tablePos[1]}, ${tablePos[2]}]`);
        } else {
            tablePos = nearbyTable.pos;
            try {
                const tableData = JSON.parse(nearbyTable.customString);
                itemCount = tableData.item_count || 0;
                console.log(`[SPAWN BATCH] Using existing table with ${itemCount} items`);
            } catch (e) {
                console.log('[SPAWN BATCH] Using existing table');
            }
        }

        // Add all spawn entries to the table
        for (const spawnEntry of spawnEntries) {
            try {
                const template = spawnTemplates[spawnEntry.class] || { ...defaultTemplate, name: spawnEntry.class };
                const itemYOffset = (template.pos && template.pos[1]) || 0.55;
                ...
                if (spawnEntry.purchaseId) {
                    try {
                        ...
                        console.error('[SPAWN BATCH] Failed to update purchase history:', err.message);
                    }
                }

                results.push({ success: true, item: spawnEntry.item, coordinates: [itemX, itemY, itemZ] });
                console.log(`[SPAWN BATCH] Added ${spawnEntry.class} at row ${row}, col ${col}`);
            } catch (itemErr) {
                console.error(`[SPAWN BATCH] Error adding ${spawnEntry.item}:`, itemErr.message);
                results.push({ success: false, item: spawnEntry.item, error: itemErr.message });
                
                if (spawnEntry.purchaseId) {
                    try {
                        await db.updatePurchaseSpawnAttempt(spawnEntry.purchaseId, true, false, itemErr.message, null);
                    } catch (err) {
                        console.error('[SPAWN BATCH] Failed to update purchase history:', err.message);
                    }
                }
        ...
        try {
            ...
            console.log('[SPAWN BATCH] Could not update table count');
        }

        // Upload spawn.json once
        const { Client } = require('basic-ftp');
        const client = new Client();
        client.ftp.verbose = false;
        ...
        try {
            ...
            await client.uploadFrom(tmpPath, FILE_PATH);
            fs.unlinkSync(tmpPath);
            
            console.log(`[SPAWN BATCH] Successfully uploaded spawn.json with ${spawnEntries.length} new items (${spawnJson.Objects.length} total objects)`);

            // Update cfggameplay.json to register spawn.json
            try {
                const downloadUrl = `${BASE_URL}/${guildConfig.nitrado_service_id}/gameservers/file_server/download?file=${encodeURIComponent(GAMEPLAY_FILE_PATH)}`;
                const downloadResp = await axios.get(downloadUrl, {
                    headers: { 'Authorization': `Bearer ${guildConfig.nitrado_token}` }
                ...
                if (!fileContent.includes(spawnJsonPath)) {
                    ...
                    if (updatedContent !== fileContent) {
                        const tmpGameplayPath = path.join(__dirname, 'logs', `cfggameplay_${Date.now()}.json`);
                        fs.writeFileSync(tmpGameplayPath, updatedContent, 'utf8');
                        await client.uploadFrom(tmpGameplayPath, GAMEPLAY_FILE_PATH);
                        fs.unlinkSync(tmpGameplayPath);
                        console.log('[SPAWN BATCH] Updated cfggameplay.json');
                    }
                }
            } catch (gameplayErr) {
                console.error('[SPAWN BATCH] Could not update cfggameplay.json:', gameplayErr.message);
            }
        ...
        console.error('[SPAWN BATCH] Error:', err.response ? err.response.data : err.message);
        throw err;
    }
}
